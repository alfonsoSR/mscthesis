...
class MatrixVisualizationManager(AnalysisManager[ResultVisualizationInput]):
    ...
    def correlation_matrix(self, source: Path) -> None:
        ...
        param_config: dict[str, bool] = raw_config["Estimation"]["parameters"]
...
class MatrixComparisonManager(ComparisonManager):
    ...
    #         params += ["$C_{D, 1}$", "$C_{D,2}$", "$C_{D,3}$", "$C_{D,4}$"]
    #     if param_config["radiation_pressure_coefficient"]:
    #         params.append("$K_1$")
    #     if param_config["k2_radiation_coefficient"]:
    #         params.append("$K_2$")
    #     if param_config["gm_phobos"]:
    #         params.append(r"$\mu_{ph}$")
    #     if param_config["gm_deimos"]:
    #         params.append(r"$\mu_{de}$")
...
class DistributionVisualizationManager(VisualizationManager):
